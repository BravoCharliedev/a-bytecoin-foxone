...
std::error_code Core::addBlock(const CachedBlock& cachedBlock, RawBlock&& rawBlock) {
  ...
  if (cachedBlock.getTypeOfBlock()==LEGACY_BLOCK) {
	  if (!currency.getBlockReward(cachedBlock.getBlock().majorVersion, blocksSizeMedian,
...
bool Core::getBlockTemplate(BlockTemplate& b, const AccountPublicAddress& adr, const BinaryArray& extraNonce,
  ...
  throwIfNotInitialized();
  AccountPublicAddress defaultAddress = boost::value_initialized<AccountPublicAddress>();
  getCurrency().parseAccountAddressString(DEFAULT_MINER_ADDRESS, defaultAddress);
  ...
  */
  ...
  std::vector<AccountPublicAddress> address;
  address.push_back(adr);
  address.push_back(defaultAddress);

  bool r = currency.constructMinerTx(b.majorVersion, height, medianSize, alreadyGeneratedCoins, transactionsSize, fee, address,
  ...
  for (size_t tryCount = 0; tryCount < TRIES_COUNT; ++tryCount) {
    r = currency.constructMinerTx(b.majorVersion, height, medianSize, alreadyGeneratedCoins, cumulativeSize, fee, address,
                                  b.baseTransaction, extraNonce, 11);
    if (!r) {
