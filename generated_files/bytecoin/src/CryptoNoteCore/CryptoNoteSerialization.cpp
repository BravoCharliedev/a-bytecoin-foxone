...
void serializeSmartContractBlock(SmartContractBlock& scb, ISerializer& serializer){

	serializer(scb.majorVersion, "majorVersion");
	serializer(scb.minorVersion, "minorVersion");
	serializer(scb.starTime, "start_time");
	serializer(scb.endTime, "end_time");
	serializer(scb.extra, "extra");
...
//*
	...
	serializer(header.typeBlock, "type_block");
	serializer(header.previousBlockHash, "prev_id");
	serializer(header.minedBy, "minedBy");
	serializer(header.nameMiner, "name_Miner");
	}
	break;
	default:{
...
//*
  ...
  block.typeBlock=LEGACY_BLOCK;}
  if (block.minorVersion >= BLOCK_MAJOR_VERSION_4){
  ...
  if (block.majorVersion >= BLOCK_MAJOR_VERSION_5 ){
  	  serializer(block.accountAddress,"public_addresss" );  //considerar eliminar
	  serializer(block.accountNumber,"account_number");     //considerar eliminar
  	  serializer(block.timestamp, "time_stamp");
	  serializer(block.dificulty, "Dificulty");

  }
	  break;
  case ACCOUNT_BLOCK:{
  	  auto parentBlockSerializer = makeParentBlockSerializer(block, false, true);
	  serializer(parentBlockSerializer, "parent_block");
	  serializer(block.accountAddress,"public_addresss" );
	  serializer(block.accountNumber,"account_number");
  	  serializer(block.timestamp, "time_stamp");
	  serializer(block.dificulty, "Dificulty");

  }
  	  break;
  case	SMART_CONTRACT_BLOCK:{
  	  serializeSmartContractBlock(block.smartContract, serializer);
  }
  	  break;
  default: {
   throw std::runtime_error("Wrong Type of Block");
   }
  	  break;
  }
//*/
