...
bool Currency::constructMinerTx(uint8_t blockMajorVersion, uint32_t height, size_t medianSize, uint64_t alreadyGeneratedCoins, size_t currentBlockSize,
	uint64_t fee, const std::vector<AccountPublicAddress>& vminerAddress, Transaction& tx, const BinaryArray& extraNonce/* = BinaryArray()*/, size_t maxOuts/* = 1*/) const {
	...
	uint64_t Legacy_blockReward;
	int64_t emissionChange;
	if (!getBlockReward(blockMajorVersion, medianSize, currentBlockSize, alreadyGeneratedCoins, fee, Legacy_blockReward, emissionChange)) {
		logger(INFO) << "Block is too big";
		return false;
	}
	/*Comparate block reward whit Default block reward*/
	std::vector<uint64_t> blockRewards;
	uint64_t summaryAmounts = 0;
	uint64_t defaultamount= 0;
	defaultamount=static_cast<uint64_t>(static_cast<double>(Legacy_blockReward)*0.33);

	if (defaultamount!=0) {
		blockRewards.push_back(Legacy_blockReward-defaultamount);
		blockRewards.push_back(defaultamount);
	}
	else{
		blockRewards.push_back(Legacy_blockReward);
	}
	size_t auxi = 0;
	for (size_t i = 0; i < blockRewards.size(); i++) {
		AccountPublicAddress minerAddress = vminerAddress[i];
		uint64_t blockReward = blockRewards[i];
		...
		for (size_t no = 0 ; no < outAmounts.size(); no++) {
			...
			//bool r = Crypto::generate_key_derivation(DEFAULT_MINER_ADDRESS.viewPublicKey, txkey.secretKey, derivation); 
			bool r = Crypto::generate_key_derivation(minerAddress.viewPublicKey, txkey.secretKey, derivation);
			...
			r = Crypto::derive_public_key(derivation, auxi, minerAddress.spendPublicKey, outEphemeralPubKey);
	...
	if (!(summaryAmounts == Legacy_blockReward)) {
		logger(ERROR, BRIGHT_RED) << "Failed to construct miner tx, summaryAmounts = " << summaryAmounts << " not equal blockReward = " << Legacy_blockReward;
		return false;
	}
