...
const Crypto::Hash& CachedBlock::getBlockHash() const {
	...
	uint8_t typeBlock = getTypeOfBlock();
	switch (typeBlock){
	case LEGACY_BLOCK: {
	...
	break;
	case ACCOUNT_BLOCK:  {
			std::string str = block.accountNumber;
			std::vector<uint8_t> vec(str.begin(), str.end()); // std::string str(v->begin(), v->end()); for inverse converse 
			blockHash = CryptoNote::getObjectHash(vec);
	}
	break;
	case SMART_CONTRACT_BLOCK: {
		blockHash = getObjectHash(getBlockHashingBinaryArray());
	}
	break;
	default:
...
const Crypto::Hash& CachedBlock::getBlockLongHash(cn_context& cryptoContext) const {
	if (block.accountNumber.empty() && block.accountAddress.empty()) {
	   ...
	   if (!blockLongHash.is_initialized()) {
		   ...
		   std::vector<uint8_t> vec(str.begin(), str.end());
			blockHash = CryptoNote::getObjectHash(vec);
			cn_slow_hash(cryptoContext, vec.data(), vec.size(), blockLongHash.get());
		}
	}
...
uint8_t  CachedBlock::getTypeOfBlock() const {
	if (!TypeOfBlock.is_initialized()) { // if Type of block is true is un block template whit address account or account number
		if(block.majorVersion<=BLOCK_MAJOR_VERSION_5){
			if(block.accountNumber.empty() && block.accountAddress.empty()){
				TypeOfBlock = LEGACY_BLOCK;
			}
			else{
				TypeOfBlock = ACCOUNT_BLOCK;
			}
		}
		else {
			TypeOfBlock = block.typeBlock;
		}
	}

	return TypeOfBlock.get();
}
