...
const char* DEFAULT_DAEMON_HOST = "94.237.65.92";
const size_t CONCURRENCY_LEVEL = std::thread::hardware_concurrency();
const uint16_t RPC_DEFAULT_MINER_PORT = 2282;
...
MiningConfig::MiningConfig(): help(false) {
  cmdOptions.add_options()
      ...
      ("account", po::value<std::string>(), "Valid cryptonote miner's address")
      ("daemon-host", po::value<std::string>()->default_value(DEFAULT_DAEMON_HOST), "Daemon host")
      ("daemon-rpc-port", po::value<uint16_t>()->default_value(static_cast<uint16_t>(RPC_DEFAULT_MINER_PORT)), "Daemon's RPC port")
	  ...
	  ("name", po::value<std::string>(), "Valid cryptonote name")
...
void MiningConfig::parse(int argc, char** argv) {
  ...
  if (options.count("name") == 0) {
    throw std::runtime_error("Specify --name option");
  }

  name = options["name"].as<std::string>();

  if (options.count("account") == 0) {
    throw std::runtime_error("Specify --account option");
  }

  miningAddress = options["account"].as<std::string>();
