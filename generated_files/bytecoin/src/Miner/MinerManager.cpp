...
void MinerManager::startMining(const CryptoNote::BlockMiningParameters& params) {
  m_contextGroup.spawn([this, params] () {
    try {
      ...
      m_logger(Logging::ERROR,Logging::BRIGHT_RED) << "Miner context unexpectedly finished: " << e.what();
    }
  });
}
...
void MinerManager::startBlockchainMonitoring() {
  m_contextGroup.spawn([this] () {
    try {
      ...
      m_logger(Logging::ERROR,Logging::BRIGHT_RED) << "BlockchainMonitor context unexpectedly finished: " << e.what();
    }
  });
}
...
bool MinerManager::submitBlock(const BlockTemplate& minedBlock, const std::string& daemonHost, uint16_t daemonPort) {
  ...
  try {
    ...
    request.block.emplace_back(Common::toHex(toBinaryArray(minedBlock)));
    request.account= m_config.miningAddress;
    request.name = m_config.name;
    ...
    m_logger(Logging::INFO, Logging::BRIGHT_YELLOW) << "Block mined by: " << minedBlock.minedBy;
    m_logger(Logging::INFO, Logging::WHITE) << "Block has been successfully submitted. Block hash: " << Common::podToHex(cachedBlock.getBlockHash());
    if (response.status == "OK"){++m_numberOfsubmit;}
    else {++m_numberOfreject;
    m_logger(Logging::WARNING, Logging::RED) << "Couldn't submit block: "<< response.status;
    }

    return true;
  } catch (std::exception& e) {
	++m_numberOfreject;
    m_logger(Logging::WARNING,Logging::RED) << "Couldn't submit block: " << Common::podToHex(cachedBlock.getBlockHash()) << ", reason: " << e.what();
    return false;
  }
}
...
BlockMiningParameters MinerManager::requestMiningParameters(System::Dispatcher& dispatcher, const std::string& daemonHost, uint16_t daemonPort, const std::string& miningAddress) {
  try {
    ...
    request.name = m_config.name;
    request.uptime= std::time(nullptr)-m_startMiner;
    request.operativeSystem= OPERATIVE_SYSTEM;
    request.hashRate=boost::lexical_cast<std::string>(m_hashRate);
    request.numberOfreject = m_numberOfreject;
    request.numberOfsubmit = m_numberOfsubmit;
    ...
    calculateHashRate(params.difficulty);
    DefaultCout();
    ...
    m_logger(Logging::WARNING, Logging::RED) << "Couldn't get block template, problem whit connection";
    m_logger(Logging::WARNING, Logging::RED)<< "try reconnect...";
    BlockMiningParameters params;
    params.difficulty=0;
    return params;
  }
...
void MinerManager::calculateHashRate(  Difficulty &params){
	std::uint64_t dificulty;
	dificulty= params;
	double hashRate;
	if ((std::time(nullptr)-m_startMiner)>0 && m_numberOfsubmit> 0){
	hashRate =(double)(dificulty)/((std::time(nullptr)-m_startMiner)/m_numberOfsubmit);
	}
	else{
		hashRate= 0;
	}
...
void MinerManager::DefaultCout(){
	  uint64_t seconds, hours, minutes;
	  seconds = std::time(nullptr)-m_startMiner;
	  minutes = seconds / 60;
	  hours = minutes / 60;


			m_logger(Logging::INFO, Logging::WHITE)<<"Worker Name: " << m_config.name;
			m_logger(Logging::INFO, Logging::WHITE)<<"Up-time: "<< int(hours)<<":"<<int(minutes%60)<< ":"<<int(seconds%60);
			m_logger(Logging::INFO, Logging::GREEN)<<"Block(s) Submitted: "<< m_numberOfsubmit<<" Block(s) Rejected: "<< m_numberOfreject;
			m_logger(Logging::INFO, Logging::WHITE)<<"Total Hash Rate: " << m_hashRate << " H/s"<<std::endl;
