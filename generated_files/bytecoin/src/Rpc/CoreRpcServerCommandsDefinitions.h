...
//-----------------------------------------------
struct stadistic_miner{
	std::string event;
	std::string minerName;
	std::string minerAccount;
	std::string uptime;
	std::string OS;
	std::string hashRate;
	std::string submittedBlocks;
	std::string rejectedBlocks;
	std::string IP;
	...
	void serialize(ISerializer &s) {
			KV_MEMBER(event)
			KV_MEMBER(minerName)
			KV_MEMBER(minerAccount)
			KV_MEMBER(uptime)
			KV_MEMBER(OS)
			KV_MEMBER(hashRate)
			KV_MEMBER(submittedBlocks)
			KV_MEMBER(rejectedBlocks)
			KV_MEMBER(IP)
			KV_MEMBER(last_seen)
		}
};
...
struct COMMAND_RPC_GETBLOCKADDRESS {
	struct request {
		std::string AccountNumber;

		void serialize(ISerializer &s) {
			KV_MEMBER(AccountNumber)
		}
	};
	struct response {
		std::string hash;
		...
		void serialize(ISerializer &s) {
			KV_MEMBER(hash)
			KV_MEMBER(AccountAddress)
			KV_MEMBER(Status)
		}
	};
};

struct COMMAND_RPC_GETBLOCKHASH_BM {
	struct request {
		std::string AccountNumber;
...
struct COMMAND_RPC_GETBLOCKTEMPLATE {
  struct request {
    ...
    std::string name;
    std::string hashRate;
    std::uint64_t numberOfsubmit;
    std::uint64_t numberOfreject;

    void serialize(ISerializer &s) {
      KV_MEMBER(reserve_size)
      KV_MEMBER(wallet_address)
	  KV_MEMBER(operativeSystem)
	  KV_MEMBER(uptime)
	  KV_MEMBER(name)
	  KV_MEMBER(hashRate)
	  KV_MEMBER(numberOfsubmit)
	  KV_MEMBER(numberOfreject)
    }
  };
  struct HTTP_RPC_request{
	  std::string data;
	  void serialize(ISerializer &s) {
	        KV_MEMBER(data)}
  };
  typedef STATUS_STRUCT  HTTP_RPC_reponse;
  struct response {
    uint64_t difficulty;
    uint32_t height;
...
struct COMMAND_RPC_GET_STATDISTIC_MINER {
	struct request {
		std::uint64_t line;
	...
	struct response {
		...
		std::vector<stadistic_miner> statistics;


		void serialize(ISerializer &s) {
			KV_MEMBER(statistics)
		}
	};
};
