...
HttpServer::HttpServer(System::Dispatcher& dispatcher, Logging::ILogger& log)
  : m_dispatcher(dispatcher), workingContextGroup(dispatcher), logger(log, "HttpServer"), lastAddress("0.0.0.0") {
...
void HttpServer::acceptLoop() {
  try {
    ...
    for (;;) {
      ...
      auto header= req.getHeaders();
          	   auto itt = header.find("x-real-ip");
          	   if (itt != header.end()){
          	        lastAddress= itt->second;}
...
std::string HttpServer::getAddress(){
	return lastAddress;
}
}
