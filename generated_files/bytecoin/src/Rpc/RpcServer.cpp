...
std::unordered_map<std::string, RpcServer::RpcHandler<RpcServer::HandlerFunction>> RpcServer::s_handlers = {
 ...
 { "/get_key_output_keys.bin", { binMethod<COMMAND_RPC_GET_KEY_OUTPUT_KEYS>(&RpcServer::onGetKeyOutputKeys), false } },
  ...
  { "/getblockaddress",{ jsonMethod<COMMAND_RPC_GETBLOCKADDRESS>(&RpcServer::on_getblockblankaddress), false } },
...
HttpServer(dispatcher, log), logger(log, "RpcServer"), m_core(c), m_p2p(p2p), m_protocol(protocol),m_dispatcher(dispatcher)
/*,m_socket(socketIP,socketPort)*/{
/*	
	std::vector<std::string>* data_sokect= &m_stadistic;
	std::thread new_thread (&RpcServer::sockectCommand, this);
	new_thread.detach();
*/
}
...
bool RpcServer::processJsonRpcRequest(const HttpRequest& request, HttpResponse& response) {
  ...
  try {
    ...
    static std::unordered_map<std::string, RpcServer::RpcHandler<JsonMemberMethod>> jsonRpcHandlers = {
	  ...
	  { "pushblock",{ makeMemberMethod(&RpcServer::on_pushblock), false } },
	  { "/getblockaddress",{ makeMemberMethod(&RpcServer::on_getblockblankaddress), false } },
      { "getlastblockheader", { makeMemberMethod(&RpcServer::on_get_last_block_header), false } },
      { "getblockheaderbyhash", { makeMemberMethod(&RpcServer::on_get_block_header_by_hash), false } },
      { "getblockheaderbyheight", { makeMemberMethod(&RpcServer::on_get_block_header_by_height), false } }, //on_get_block_hash_block_manager
	  { "/getblockhash",{ makeMemberMethod(&RpcServer::on_get_block_hash_block_manager), false } }
	};
...
bool RpcServer::on_getblockblankaddress(const COMMAND_RPC_GETBLOCKADDRESS::request & req, COMMAND_RPC_GETBLOCKADDRESS::response & res) {
	std::string reqs = req.AccountNumber;
	if (reqs.empty() || reqs.size() != 10) {
		throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_WRONG_PARAM, "Wrong parameters, expected height" };
	}
	std::vector<uint8_t> vec(reqs.begin(), reqs.end());
	Crypto::Hash blockHash = CryptoNote::getObjectHash(vec);
	if (blockHash == NULL_HASH) {
		throw JsonRpc::JsonRpcError{
			CORE_RPC_ERROR_CODE_TOO_BIG_HEIGHT,
			std::string("The Account Number: " + reqs)
		};
	}
	try {
		res.AccountAddress = m_core.getBlockByHash(blockHash).accountAddress;
		res.hash= Common::podToHex(blockHash);
	}
	catch (const std::exception& e) {
...
//---------------------------------------------------------------------------------
  std::string Data=";GET_PARAMS;"+address+";"+req.name+";"+req.wallet_address+";"+std::to_string(req.uptime)+";"+req.operativeSystem;
  stadistic_miner Tstadistic;
  Tstadistic.minerAccount = req.wallet_address;
  Tstadistic.event = "GET_PARAMS";
  Tstadistic.minerName = req.name;
  Tstadistic.hashRate = req.hashRate ;
  Tstadistic.rejectedBlocks= std::to_string(req.numberOfreject);
  Tstadistic.submittedBlocks= std::to_string(req.numberOfsubmit);
  Tstadistic.OS= req.operativeSystem;
  Tstadistic.uptime= std::to_string(req.uptime);
  Tstadistic.IP=address;
  //Tstadistic.last_seen;

  auto Jason_data = storeToJsonValue(Tstadistic);
  std::string jdata =Jason_data.toString()+"\r\n";
/*
  mtx.lock();
  std::cout<<"mutex on "<<this->m_stadistic.size()<<std::endl;
  if (this->m_stadistic.size()< 100 ){
	  this->m_stadistic.push_back(jdata);
  }
  else {
	  this->m_stadistic.erase(m_stadistic.begin());
	  this->m_stadistic.push_back(jdata);
  }
  mtx.unlock();*/
// ---------------------------------------------------------
  ...
  AccountPublicAddress acc = boost::value_initialized<AccountPublicAddress>();
   ...
   try {
	   std::vector<uint8_t> vec(req.wallet_address.begin(), req.wallet_address.end());
	   Crypto::Hash blockHash = CryptoNote::getObjectHash(vec);
	   miner_address = m_core.getBlockByHash(blockHash).accountAddress;
	   
   }
   catch (const std::exception&)
   {
	   miner_address = DEFAULT_MINER_ADDRESS;
	   logger(WARNING) << "the account Number not found "<<ENDL;
   }



  // std::string miner_address = req.wallet_address;
...
bool RpcServer::on_submitblock(const COMMAND_RPC_SUBMITBLOCK::request& req, COMMAND_RPC_SUBMITBLOCK::response& res) {
  ...
  else if (submitResult == error::AddBlockErrorCode::ALREADY_EXISTS){
	  throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_BLOCK_NOT_ACCEPTED, "the block ALREADY_EXISTS" };
	  res.status = "the block ALREADY_EXISTS";
  }

  else if (submitResult == error::AddBlockErrorCode::REJECTED_AS_ORPHANED){
	  throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_BLOCK_NOT_ACCEPTED, "REJECTED_AS_ORPHANED" };
	  res.status = "REJECTED_AS_ORPHANED";
  }
  else if (submitResult == error::AddBlockErrorCode::DESERIALIZATION_FAILED){
	  throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_BLOCK_NOT_ACCEPTED, "DESERIALIZATION_FAILED" };
	  res.status = "DESERIALIZATION_FAILED";
  }
  else{
  }
...
bool RpcServer::on_pushblock(const COMMAND_RPC_PUSHBLOCK::request & req, COMMAND_RPC_PUSHBLOCK::response & res){
	
	if(req.AccountAddress.size()!=95 || req.AccountNumber.size()!=10){
		throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_WRONG_PARAM, "Wrong param" };
		return true;
		res.status = "Wrong param";
	}

	CryptoNote::BlockTemplate block = boost::value_initialized<CryptoNote::BlockTemplate>();

	Transaction tx;
	uint32_t index= m_core.getTopBlockIndex();
	BaseInput in;
	in.blockIndex= index+1;
	tx.inputs.push_back(in);
	tx.version=1;
	in.blockIndex = index;

	block.typeBlock= ACCOUNT_BLOCK;
	block.previousBlockHash = m_core.getBlockHashByIndex(index);
	block.parentBlock.transactionCount = 1;
	block.majorVersion = 5;
	block.minorVersion = 4;
	block.accountAddress = req.AccountAddress;
	block.baseTransaction = tx;
	block.minedBy = "";
	block.accountNumber = req.AccountNumber;
	block.timestamp = std::time(nullptr);
	BinaryArray blockblob=toBinaryArray(block);
	

	auto submitResult = m_core.submitBlock(toBinaryArray(block));

	if (submitResult == error::AddBlockErrorCode::ADDED_TO_MAIN
		|| submitResult == error::AddBlockErrorCode::ADDED_TO_ALTERNATIVE_AND_SWITCHED) {
		NOTIFY_NEW_BLOCK::request newBlockMessage;
		newBlockMessage.b = prepareRawBlockLegacy(std::move(blockblob));
		newBlockMessage.hop = 0;
		newBlockMessage.current_blockchain_height = m_core.getTopBlockIndex() + 1; //+1 because previous version of core sent m_blocks.size()
		res.status = "Submit Block";
		m_protocol.relayBlock(newBlockMessage);
		return true ;
	}

	res.status = " Failed to submit block... ";
	return true;
...
bool RpcServer::on_get_block_hash_block_manager(const COMMAND_RPC_GETBLOCKHASH_BM::request & req, COMMAND_RPC_GETBLOCKHASH_BM::response & res)
{
	std::vector<uint8_t> vec(req.AccountNumber.begin(), req.AccountNumber.end());
	Crypto::Hash blockHash = CryptoNote::getObjectHash(vec);
	bool r;
	try {
		r = m_core.hasBlock(blockHash);
	}
	catch (const std::exception& e) {
	...
	if (!r) {
		res.Status = "account don�t found ";
		return true;
	}
	res.hash = Common::podToHex(blockHash);
...
/*
void RpcServer::sockectCommand(){
	std::vector<std::string> Data;
	for (;;){
		usleep(500);
		mtx.lock();
		Data=m_stadistic ;
		this->m_stadistic.clear();
		mtx.unlock();

		if (Data.size()>0){
			for (auto&& data : Data){
				if (!m_socket.send_data(data)){
					if (m_socket.conect()){
						if(!m_socket.send_data(data)){
							std::cout<<"dont sent data "<<std::endl;
							continue;
						}
					}
				}
